...
export default function HomeIndustryExpertsSectionPage() {
  const [sectionTitle, setSectionTitle] = useState("Industry Experts");
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) setToast({ message: "Industry experts section saved successfully", type: "success" });
      else setToast({ message: data.error || "Failed to save", type: "error" });
    } catch (error) {
      setToast({ message: "Error saving industry experts section", type: "error" });
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="min-h-screen bg-muted">
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Industry Experts Manager</h1>
            <p className="text-sm text-muted-foreground mt-1">Manage homepage industry expert profiles</p>
          </div>
